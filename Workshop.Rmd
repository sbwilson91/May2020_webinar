...
There are online repositories where published datasets are stored. One such is the Gene Expression Omnibus (GEO) where our group has deposited a dataset that will work with today. You can download and unzip these files through the website interactively, or you can try and download within R.  

```{r}
#install.packages("GEOquery")
GEOquery::getGEOSuppFiles(GEO = "GSE119561", baseDir = "data/", makeDirectory = T)
untar(tarfile = "data/GSE119561/GSE119561_RAW.tar",
      exdir = "data/GSE119561/extracted")
```

### Loading 10x data

There are two forms of output I have worked with, .tsv and .mtx files containing parts of the data (cells, genes, counts), or .h5 files containing all data. We use these to create a "Seurat" object, which is the format the Seurat package works with. Both will produce the same outcome.  

We are only going to use the "E6" organoid sample which was at 25 days of age. We need to rename these files as well, as the Read10X function requires the files to be specifically named:  

```{r}
list.files("data/GSE119561/extracted/", full.names = T)
dir.create("data/D25/")
file.copy(from = list.files("data/GSE119561/extracted/", full.names = T)[1:3],
          to = c("data/D25/barcodes.tsv.gz",
                 "data/D25/features.tsv.gz",
                 "data/D25/matrix.mtx.gz"))
list.files("data/D25/")
data.1 <- Read10X(data.dir = "data/D25")

#data.1 <- Read10X_h5(filename = #"/group/kidn1/Group-Little_MCRI/Data/SingleCellRNASeq/ReporterOrganoid/MAFB_GATA3_reporter/outs/filtere#d_gene_bc_matrices_h5.h5")

seurat <- CreateSeuratObject(data.1, project = "D25_Organoid")
seurat
```
